...
from data_model import SEISData, TWINSData
...
def main():
    ...
    parser.add_argument('--range', type=int, default=DEFAULT_SOL_RANGE,
                        help=f'데이터를 처리할 솔 수 또는 일수 (기본값: {DEFAULT_SOL_RANGE})')
    ...
    # SEIS 데이터 처리
    seis_data = SEISData(start_sol, sol_range, channel, data_paths)
    seis_data.process_data(minfreq, maxfreq)

    # TWINS 데이터 처리
    twins_data = TWINSData(start_sol, sol_range, data_paths)
